...
**Regional specialists** (`figures/phaC_regional_specialists.png`) — clusters geographically concentrated in one place (`geo_mean_resultant_length > 0.85`) rather than cosmopolitan. **This collapsed hard on the second pass: of the 11 clusters that survived the first audit, 9 no longer exist at all — their entire membership was hits recruited by the 152 newly-excluded references.** Only **2 remain**: `Robiginitomaculum_A` (mid North Atlantic, ~173m average depth, n=16) and `Algiphilus` (near Australia/Southern Ocean, ~29m average depth, n=15). This figure has now gone from 16 hand-picked clusters (pre-audit) → 11 (first audit) → 2 (this fix) — each round removing more of what turns out to have been reference-driven noise rather than real, tightly-localized biology. A fresh candidate search from scratch (not hand-picked survivors of prior rounds) would be needed to say anything more about regional phaC specialists going forward.
...
**REDSEA-S09-B13 case study** (`figures/redsea_depth_clusters.png`) — **this case study lost most of its evidentiary basis.** Only 6 REDSEA-S09-B13 genomes with resolved depth remain (the largest cluster before this fix had 89 genomes alone), across 2 clusters spanning a narrow 270-480m band — nowhere near the old 0-5,100m range the "same cluster persists from surface to deep" finding was based on. That finding is no longer supportable from this genus with the current data; it is not restated here as still true. Worth re-picking a different genus for this specific case study if the depth-vs-cluster question is still wanted, rather than continuing to lean on REDSEA-S09-B13.
