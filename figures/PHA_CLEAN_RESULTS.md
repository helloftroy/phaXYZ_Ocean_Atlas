...
- `structure_prediction/run_foldseek_search.sh` — builds a Foldseek database from the folded reference structures, then searches the uncertain-tier and positive-control structures against it (`easy-search`, TM-align mode for a direct TM-score). Needs real `.pdb` output from the ESMFold jobs first; not runnable yet.
...
### 6.4 Next steps (not yet started)
...
3. Once ESMFold structures land: run `structure_prediction/run_foldseek_search.sh` — this is the actual "does it look like a real PhaC" test (structural alignment, not sequence).
...
**Files:** `structure_prediction/build_fold_inputs.py`, `run_esmfold.py`, `run_foldseek_search.sh`, `uncertain_cluster_representatives.faa` (4,992), `positive_control_representatives.faa` (428), `audited_reference_set.faa` (1,875), `fold_manifest.tsv`; `cluster/install_esmfold.sh`, `run_esmfold.sbatch`, `install_foldseek.sh`, `install_colabfold.sh`, `run_colabfold.sbatch`.
