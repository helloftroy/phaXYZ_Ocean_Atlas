...
#   ./download_gopc.sh mags       # 43191.all_MAGs.tar.gz -- ~31GB. Cluster recommended.
...
download_mags() {
  local dest="${RAW_DIR}/43191.all_MAGs.tar.gz"
  local md5_file="${RAW_DIR}/md5.txt"
  if [ ! -f "${md5_file}" ]; then
    echo "md5.txt not present locally yet -- fetching it first to get the expected checksum." >&2
    download_md5
  ...
  local expected
  expected="$(grep -F '43191.all_MAGs.tar.gz' "${md5_file}" | awk '{print $1}')"
  if [ -z "${expected}" ]; then
    echo "Could not find an md5 for 43191.all_MAGs.tar.gz in ${md5_file} -- check its format hasn't changed upstream." >&2
    exit 2
  fi

  resumable_download "${BASE_URL}/43191.all_MAGs.tar.gz" "${dest}"
  local size; size="$(file_size_bytes "${dest}")"
  ...
  write_manifest_row "${MANIFEST}" "43191.all_MAGs.tar.gz" "${BASE_URL}/43191.all_MAGs.tar.gz" "${size}" "md5" "${expected}" "${status}"
  if [ "${status}" = "FAILED_verification" ]; then
    echo "Checksum verification FAILED -- see manifest and re-run to retry (curl -C - will resume, not restart)." >&2
    exit 1
  fi
...
case "${TARGET}" in
  ...
  mags) download_mags ;;
  all) download_md5; download_geneset; download_mags ;;
